...
function getEntitlementsPath(projectRoot, {
  ...
  const project = (0, _Xcodeproj().getPbxproj)(projectRoot, nativeDir);
  ...
  const entitlementsPath = getEntitlementsPathFromBuildConfiguration(projectRoot, xcBuildConfiguration, nativeDir);
  return entitlementsPath && _fs().default.existsSync(entitlementsPath) ? entitlementsPath : null;
}
function getEntitlementsPathFromBuildConfiguration(projectRoot, xcBuildConfiguration, nativeDir = 'ios') {
  const entitlementsPathRaw = xcBuildConfiguration?.buildSettings?.CODE_SIGN_ENTITLEMENTS;
  if (entitlementsPathRaw) {
    return _path().default.normalize(_path().default.join(projectRoot, nativeDir, (0, _string().trimQuotes)(entitlementsPathRaw)));
  } else {
    return null;
  }
}
function ensureApplicationTargetEntitlementsFileConfigured(projectRoot, nativeDir = 'ios') {
  const project = (0, _Xcodeproj().getPbxproj)(projectRoot, nativeDir);
  const projectName = (0, _Xcodeproj().getProjectName)(projectRoot, nativeDir);
  ...
  for (const [, xcBuildConfiguration] of buildConfigurations) {
    const oldEntitlementPath = getEntitlementsPathFromBuildConfiguration(projectRoot, xcBuildConfiguration, nativeDir);
    ...
    const entitlementsPath = _path().default.resolve(projectRoot, nativeDir, entitlementsRelativePath);
